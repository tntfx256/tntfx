...
import type { Actionable, Actions, Any, ClassAndChildren, Nullable, OnAction } from "@tntfx/core";
import { logger } from "@tntfx/core";
import type { ToolbarSectionProps } from "./toolbar-section";
import { ToolbarSection } from "./toolbar-section";
import { Button } from "../../button";

type ActionBarProps<T extends string = string> = Actionable<T> & ToolbarSectionProps;

export function ActionBar<T extends string = string>(props: ClassAndChildren<ActionBarProps<T>>) {
  const { actions, onAction, ...boxProps } = props;

  const actionableItems = useActions(actions, onAction);

  if (actionableItems) {
    return <ToolbarSection {...boxProps}>{actionableItems}</ToolbarSection>;
  }
